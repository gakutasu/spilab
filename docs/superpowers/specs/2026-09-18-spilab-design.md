...
## 15. 分野の再構成と受検形式（2026-09-21）

- `Category` に `english` を追加。`TOPICS` の各分野に `formats: ('testcenter'|'paper')[]` を持たせる
- 設定 `formats.testcenter` / `formats.paper` / `includeEnglish`。`selectQuestions` は `filterPool` で有効な分野に絞ってから抽選。英語 ON のときは出題数のおよそ 1/6（最低 1 問）を英語に割り当て、残りを言語・非言語で分ける
- 新分野：言語 = 熟語の成り立ち・語句の用法（既存 2 問を移動）・空欄補充、非言語 = 料金割引・代金精算、命題・論理、整数・数列、方程式・不等式、図形、物の流れと比率（ペーパー）、グラフの領域（ペーパー）、英語 = 同意語・反意語、空欄補充、英英辞典、誤文訂正、和文英訳、長文読解
- 装置と回路は図が必須のため対象外
- 問題 ID は変更しない（分野の移動は topic フィールドのみ）
