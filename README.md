...
- **問題数**：33 分野・311 問（言語 9 / 非言語 18 / 英語 6）。全問、作成後に独立して解き直す検証済み
- **受検形式**：テストセンター / ペーパーテスト / 英語（ENG）の有無を設定で切替
...
| Claude / OpenAI API（任意） | 採点後の「AIに質問」。利用者自身の API キーでブラウザから直接呼び出し |

## 使い方

1. ホームで「今日のSPIを始める」（既定 7 問、設定で 5〜15 問）。設定で受検形式（テストセンター / ペーパー）と英語の有無を切替
2. 「問題をはじめる」でタイマー開始 → A〜D または「わからない」→「解答する」
3. 採点・目安時間との比較・詳細解説を読んで次へ（API キーを登録すると AI に質問できる）
